...
private _endPointHelper = "Land_PencilGreen_F" createVehicleLocal [0,0,0];
...
private _mouseClickID = [_player, "DefaultAction", {pbw_mines_isPlacingCable isEqualTo PLACE_WAITING}, {pbw_mines_isPlacingCable = PLACE_APPROVE}] call ace_common_fnc_addActionEventHandler;
...
[{
    ...
    private _pos = ((eyePos _player) vectorAdd ((getCameraViewDirection _player) vectorMultiply ENDPOINTHELPER_DISTANCE));
    _pos set [2, ENDPOINTHELPER_HEIGHT];
    _endPointHelper setPosATL _pos;
    _endPointHelper setVectorDirAndUp [[0,0,1], [0,1,0]];
    
    private _dm12Pos = getPos _dm12;
    private _endPointPos = getPos _endPointHelper;
    ...
    while {true} do {
        _vec = _endPointPos vectorDiff _begin;
        ...
        private _color = if (_cableLength <= _maxCableLength) then {CABLE_COLOR_GOOD} else {CABLE_COLOR_BAD};
    ...
    if (_cableLength > _maxCableLength && {pbw_mines_isPlacingCable isEqualTo PLACE_APPROVE}) then {pbw_mines_isPlacingCable = PLACE_WAITING};
    ...
    if (pbw_mines_isPlacingCable isNotEqualTo PLACE_WAITING) exitWith {
        ...
        if (pbw_mines_isPlacingCable isEqualTo PLACE_APPROVE) then {
            ...
            _dm12 setVariable ["pbw_mines_isTriggerSet", true, true];
